;	// =====================================================================
;	// path.asm -- current-path helpers
;	// =====================================================================
;	// Public API:
;	//   path_init            reset current path to root
;	//   chdir HL=path        change directory (absolute or relative)
;	//   path_get HL=buf      copy current path as ASCIIZ
;	//   path_join HL=path DE=buf  build an absolute path for a filename


PATH_MAX		equ 256

;	// path_init : reset the current path to root.
path_init:
	ld hl,cwd_cur
	ld (hl),'/'
	inc hl
...
;	// path_get : HL = destination buffer, copy the current path as ASCIIZ.
path_get:
	ex de,hl
	ld hl,cwd_cur
	jp cwd_copy_string

;	// path_join : HL = filename/path, DE = destination buffer.
...
;	// chdir : HL = path (absolute or relative).
;	//   Validates the directory, then commits it as the current path.
chdir:
	ld de,cwd_tmp
	ld a,1
